...
int sys_thread_destroy(int sysno, u_int threadid)
{
    ...
    {
        tmpt = LIST_FIRST(&t->tcb_joined_list);
        LIST_REMOVE(tmpt, tcb_joined_link);
        if (tmpt->tcb_join_value_ptr)
        {
            *(tmpt->tcb_join_value_ptr) = t->tcb_exit_ptr;
            t->tcb_exit_ptr = NULL;
        } //**val in pthread_join
...
{
    // 默认新增的线程放到当前进程（curenv）中，并且优先级是curenv->threads[0] 的优先级
    int r;
    struct Tcb *t;
...
int sys_thread_join(int sysno, u_int threadid, void **value_ptr)
{
    ...
    struct Tcb *t;
    int r;
    printf("in sys_thread_join: thread id is 0x%x\n", curtcb->thread_id);
    printf("in sys_thread_join: finded id is 0x%x\n", t->thread_id);
    r = threadid2tcb(threadid, &t);
    // printf("asfasf  r = %d\n",r);
    if (r < 0)
        return r;
    ...
    if (t->tcb_status == ENV_FREE)
    {
        ...
        printf("that thread is free, return 0 successfully!\n");
        return 0;
    }
    printf("in sys_thread_join: father id is 0x%x\n", t->thread_id);
    ...
    printf("sys_thread_join end with success\n");
    sys_yield();
    return 0;
}
...
void sys_yield(void)
{
    ...
    // need to use bcopy, because env_run() will use TIMESTACK instead of KERNEL_SP
    sched_yield();
}
                ...
                u_int perm)
{
    ...
    ret = envid2env(dstid, &dstenv, 0); // not sure about it
    if (ret < 0)
        return ret;
...
int sys_env_alloc(void)
{
    ...
    if (curenv)
        e->env_threads[0].tcb_pri = curenv->env_threads[0].tcb_pri; // 全都变成threads[0]了
    else
